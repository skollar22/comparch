...
begin
    ...
    switch_in  <= '1' when opcode = OP_SWLD else
                  '0';
-- new signals
-- led_write <- true only for OP_DISP
-- the beq mux control should only be controlled by the alu output signal - maybe have a separate eq signal come out of the alu????
-- but it should also only be 1 for the beq opcode
